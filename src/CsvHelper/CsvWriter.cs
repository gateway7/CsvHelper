...
namespace CsvHelper
{
    ...
    public class CsvWriter : ICsvWriter
    {
        ...
        private readonly List<string> _currentRecord = new List<string>();

        private readonly Dictionary<Type, Delegate> _typeActions = new Dictionary<Type, Delegate>();

        private readonly ICsvWriterConfiguration _configuration;

        private bool _isDisposed;

        private ICsvSerializer _serializer;

        private bool _hasHeaderBeenWritten;

        private bool _hasRecordBeenWritten;
        ...
        /// Gets the serializer.
        /// </summary>
        public virtual ICsvSerializer Serializer => _serializer;
        ...
        /// </summary>
        public virtual List<string> CurrentRecord => _currentRecord;

        /// <summary>
        /// Gets the configuration.
        /// </summary>
        public virtual ICsvWriterConfiguration Configuration => _configuration;
        ...
        public CsvWriter(ICsvSerializer serializer, bool leaveOpen)
        {
            ...
            _configuration = (ICsvWriterConfiguration)serializer.Configuration;
        ...
        {
            var shouldQuote = _configuration.QuoteAllFields;

            if (field != null && _configuration.TrimFields)
            {
                field = field.Trim();
            }

            if (!_configuration.QuoteNoFields && !string.IsNullOrEmpty(field))
            {
                if (shouldQuote // Quote all fields
                    || field.Contains(_configuration.QuoteString) // Contains quote
                    || field[0] == ' ' // Starts with a space
                    || field[field.Length - 1] == ' ' // Ends with a space
                    || field.IndexOfAny(_configuration.QuoteRequiredChars) > -1 // Contains chars that require quotes
                    || _configuration.Delimiter.Length > 0 && field.Contains(_configuration.Delimiter) // Contains delimiter
                    || _configuration.AllowComments && _currentRecord.Count == 0 && field[0] == _configuration.Comment) // Comments are on first field starts with comment char
                {
                    shouldQuote = true;
                }
            }
        ...
        public virtual void WriteField(string field, bool shouldQuote)
        {
            ...
            {
                field = field.Replace(_configuration.QuoteString, _configuration.DoubleQuoteString);
            }

            if (_configuration.UseExcelLeadingZerosFormatForNumerics && !string.IsNullOrEmpty(field) && field[0] == '0' && field.All(char.IsDigit))
            {
                field = "=" + _configuration.Quote + field + _configuration.Quote;
            }
            else if (shouldQuote)
            {
                field = _configuration.Quote + field + _configuration.Quote;
        ...
        public virtual void WriteField<T>(T field, ITypeConverter converter)
        {
            ...
            propertyMapData.TypeConverterOptions = TypeConverterOptions.Merge(_configuration.TypeConverterOptionsFactory.GetOptions(type), propertyMapData.TypeConverterOptions);
        ...
        public virtual void NextRecord()
        {
            try
            {
                _serializer.Write(_currentRecord.ToArray());
            ...
            catch (Exception ex)
            {
                ...
                ExceptionHelper.AddExceptionData(csvHelperException, Row, null, null, null, _currentRecord.ToArray());
        ...
        {
            WriteField(_configuration.Comment + comment, false);
        }
        ...
        public virtual void WriteRecord<T>(T record)
        {
            ...
            try
            {
                ...
                _hasRecordBeenWritten = true;
            }
            catch (Exception ex)
            {
                var csvHelperException = ex as CsvHelperException ?? new CsvWriterException("An unexpected error occurred.", ex);
                ExceptionHelper.AddExceptionData(csvHelperException, Row, record.GetType(), null, null, _currentRecord.ToArray());
        ...
        public virtual void WriteRecords(IEnumerable records)
        {
            ...
            try
            {
                ...
                foreach (var record in records)
                {
                    ...
                    else
                    {
                        ...
                        var isPrimitive = recordType.GetTypeInfo().IsPrimitive;
                        if (_configuration.HasHeaderRecord && !_hasHeaderBeenWritten && !isPrimitive)
                        {
                            WriteHeader(recordType);
                            NextRecord();
                        }
            ...
            {
                var csvHelperException = ex as CsvHelperException ?? new CsvWriterException("An unexpected error occurred.", ex);
                ExceptionHelper.AddExceptionData(csvHelperException, Row, recordType, null, null, _currentRecord.ToArray());
        ...
        protected virtual Expression CreatePropertyExpression(Expression recordExpression, CsvClassMap mapping, CsvPropertyMap propertyMap)
        {
            ...
            foreach (var refMap in mapping.ReferenceMaps)
            {
                ...
                Type propertyType;
                if (isValueType && !isGenericType && !_configuration.UseNewObjectForNullReferenceMembers)
                {
        ...
        protected virtual Delegate GetWriteRecordAction<T>(T record)
        {
            ...
            if (!_typeActions.TryGetValue(type, out Delegate action))
            {
        ...
        protected virtual Delegate CreateWriteRecordAction<T>(Type type, T record)
        {
            ...
            return type.GetTypeInfo().IsPrimitive ? CreateActionForPrimitive(type) : CreateActionForObject(type);
        }
        ...
        protected virtual Delegate CreateActionForObject(Type type)
        {
            ...
            foreach (var propertyMap in properties)
            {
                ...
                else
                {
                    ...
                    fieldExpression = CreatePropertyExpression(recordParameter, _configuration.Maps[type], propertyMap);
                    ...
                    propertyMap.Data.TypeConverterOptions = TypeConverterOptions.Merge(
                                                                                       _configuration.TypeConverterOptionsFactory.GetOptions(propertyMap.Data.Member.MemberType()), propertyMap.Data.TypeConverterOptions);
        ...
        protected virtual Delegate CreateActionForExpandoObject(ExpandoObject obj)
        {
            ...
            _typeActions[typeof(ExpandoObject)] = action;
        ...
        /// <returns>
        /// A value indicating if the property/field can be written.
        /// True if the property/field can be written, otherwise false.
        /// </returns>
        protected virtual bool CanWrite(CsvPropertyMap propertyMap)
        {
            ...
            if (propertyMap.Data.Member is PropertyInfo property)
            {
                cantWrite = cantWrite ||
                            ...
                            property.GetGetMethod() == null && !_configuration.IncludePrivateMembers ||
